...
public class FlowerFlake {
	 ...
	 * determined.
	 */
	static FlowerFlake createFlake(Context context, float xRange, Bitmap originalBitmap) {
		FlowerFlake flake = new FlowerFlake();
		// Size each flake with a width between 5 and 55 and a proportional
		// height
		int size = BaseUtils.dip2px(context, 25);
		flake.width = (int) (size + (float) Math.random() * 10);
