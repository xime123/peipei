...
public class RoseFlake {
	 ...
	 */
	static RoseFlake createFlake(Context context, float xRange, Bitmap originalBitmap) {
		RoseFlake flake = new RoseFlake();
		...
		flake.y = 0 - (flake.height + (float) Math.random() * flake.height + 300);

		// Each flake travels at 50-200 pixels per second
		flake.speed = 300 + (float) Math.random() * 150;
