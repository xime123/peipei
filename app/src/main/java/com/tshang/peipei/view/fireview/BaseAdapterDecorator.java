...
import com.tshang.peipei.view.fireview.DynamicListView.Swappable;
...
public abstract class BaseAdapterDecorator extends BaseAdapter implements SectionIndexer, DynamicListView.Swappable {
	 ...
	 * parent will prevent touch-events from reaching the list-view.
	 * 
	 * Call this method with the value 'true' to fix this behavior. Note that
	 * this will prevent the parent from scrolling horizontally when the user
	 * touches anywhere in a list-item.
	 * 
	 ...
	 * parent will prevent touch-events from reaching the list-view.
	 * 
	 * If a list-item view has a child with the given resource-ID, the user can
	 * still swipe the list-item by touching that child. If the user touches an
	 * area outside that child (but inside the list-item view), then the swipe
	 * will not happen and the parent will do its job instead (scrolling
	 * horizontally).
	 * 
	 * @param childResId
