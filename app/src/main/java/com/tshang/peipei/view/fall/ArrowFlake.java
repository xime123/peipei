...
public class ArrowFlake {
	 ...
	 * determined.
	 */
	static ArrowFlake createFlake(Context context, float xWidth, float xHeight, Bitmap originalBitmap) {
		ArrowFlake flake = new ArrowFlake();
		// Size each flake with a width between 5 and 55 and a proportional
		// height
		int size = BaseUtils.dip2px(context, 7);
		...
		// range
		flake.x = (float) Math.random() * (xWidth - flake.width);
		...
		// and 45
		// degrees per second
		flake.rotation = (xHeight / 2) / (flake.x - xWidth / 2);
		if (flake.rotation > 45) {
			flake.rotation = 45;
		} else if (flake.rotation < -45) {
			flake.rotation = -45;
		}
