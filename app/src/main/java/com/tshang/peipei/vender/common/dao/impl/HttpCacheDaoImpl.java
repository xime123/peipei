package com.tshang.peipei.vender.common.dao.impl;
...
import com.tshang.peipei.vender.common.constant.DbConstants;
...
public class HttpCacheDaoImpl implements HttpCacheDao {
	...
	public HttpResponse getHttpResponse(String url) {
		...
		synchronized (HttpCacheDaoImpl.class) {
			Cursor cursor = sqliteUtils.getRDb().query(DbConstants.HTTP_CACHE_TABLE_TABLE_NAME, null, appWhere.toString(), appWhereArgs, null, null,
					null);
			if (cursor == null) {
				return null;
			}
	...
	public Map<String, HttpResponse> getHttpResponsesByType(int type) {
		...
		synchronized (HttpCacheDaoImpl.class) {
			Cursor cursor = sqliteUtils.getRDb().query(DbConstants.HTTP_CACHE_TABLE_TABLE_NAME, null, whereClause.toString(), whereClauseArgs, null,
					null, null);
