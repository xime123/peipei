...
public class Recorder implements OnCompletionListener, OnErrorListener {
	...
	public static final String SAMPLE_DEFAULT_DIR = "/PeiPei/Audio/Temp";
	...
	public static Recorder instance = null;
	...
	public void saveState(Bundle recorderState) {
		if (recorderState != null && mSampleFile != null) {
	...
	public int sampleLength() {
		if (mSampleLength > BAConstants.AUDIO_LENGTH) {
			mSampleLength = BAConstants.AUDIO_LENGTH;
		}
	...
	private int isPlaySmileVoice = -1;//如果是播放队列中的后宫表情语音，就会大于等于0

	public void startPlayback(String path, int isPlaySmileVoice) {
		this.isPlaySmileVoice = isPlaySmileVoice;
		if (state() == PLAYING_PAUSED_STATE) {
			mSampleStart = System.currentTimeMillis() - mPlayer.getCurrentPosition();
			mPlayer.start();
			setState(PLAYING_STATE);
	...
	public boolean onError(MediaPlayer mp, int what, int extra) {
		...
		if (isShow) {
			...
			event.setFlag(NoticeEvent.NOTICE78);
	...
	public void onCompletion(MediaPlayer mp) {
		stop();
		//		handler.sendEmptyMessage(BAConstants.HandlerType.STATE_END_PLAYING);
		if (isShow) {
			NoticeEvent event = new NoticeEvent();
			event.setFlag(NoticeEvent.NOTICE78);
			EventBus.getDefault().post(event);
		...
		HandlerUtils.sendHandlerMessage(handler, BAConstants.HandlerType.STATE_END_PLAYING, isPlaySmileVoice, isPlaySmileVoice);
	}
	...
	public void setError(int error) {
		...
		HandlerUtils.sendHandlerMessage(handler, HandlerValue.CHAT_SMILE_VOICE_PLAY_ERROR_VALUE, isPlaySmileVoice, isPlaySmileVoice);
	}
}
