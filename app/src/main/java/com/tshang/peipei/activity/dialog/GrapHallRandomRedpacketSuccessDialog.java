 ...
 * @version V1.0   
 */
public class GrapHallRandomRedpacketSuccessDialog extends Dialog implements OnDismissListener, android.view.View.OnClickListener {
	...
	public GrapHallRandomRedpacketSuccessDialog(Activity activity) {
		super(activity);
		this.activity = activity;
	}

	public GrapHallRandomRedpacketSuccessDialog(Activity activity, int theme, BroadcastRedPacketInfo grabHallRedPacketInfo, BAHandler mHandler) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.dialog_grap_hall_random_redpacket_success);
		...
		if(grabHallRedPacketInfo != null){
			if (grabHallRedPacketInfo.type.intValue() == 0) {
				if(grabHallRedPacketInfo.userInfo != null){
					...
					tv_sender.setText(new String(grabHallRedPacketInfo.userInfo.nick) + activity.getString(R.string.str_fight));	
				}
			} else {
				iv_avatar.setImageResource(R.drawable.logo_rounded);
				tv_sender.setText(activity.getString(R.string.str_official) + activity.getString(R.string.str_fight));
			}
			String totalmoney = "";
...
//			}
		...
		GrapHallRandomRedpacketSuccessDialog.this.setOnDismissListener(this);
	}
