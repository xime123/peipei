...
public class SpaceCustomHzListViewAdapter extends ArrayListAdapter<PhotoInfo> {
	...
	public SpaceCustomHzListViewAdapter(Activity context, int whosUid, int sex) {
		super(context);
		this.whosUid = whosUid;
		this.sex = sex;
		int mItemWidth = (BasePhone.getScreenWidth(context) - BaseUtils.dip2px(context, 21)) / 6;
		mParams = new RelativeLayout.LayoutParams(BasePhone.getScreenWidth(context) / 6, mItemWidth);
		mImgParams = new RelativeLayout.LayoutParams(BasePhone.getScreenWidth(context) / 6, mItemWidth);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (mList != null && position < mList.size()) {
			...
			if (photo != null) {
				String key = new String(photo.key) + BAConstants.LOAD_210_APPENDSTR;
