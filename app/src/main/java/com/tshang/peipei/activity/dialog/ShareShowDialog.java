...
public class ShareShowDialog extends Dialog implements android.view.View.OnClickListener, IHeadToHttp, ILoadingPicHttp {
	...
	public ShareShowDialog(Activity context, int otherUid, String nick, String url, BAHandler mHandler, Tencent mTencent, IWXAPI mWXapi) {
...
//			});
		...
		try {
			if (mWeiboShareAPI.isWeiboAppSupportAPI()) {
				...
				if (supportApi >= 10351 /*ApiUtils.BUILD_INT_VER_2_2*/) {
					b = mWeiboShareAPI.sendRequest(context,spaceCustomBiz2.sendMultiMessage(context.getString(R.string.str_show_share_mm), bitmap, url,
							String.format(context.getString(R.string.str_show_share_content), otherUid)));
				} else {
					b = mWeiboShareAPI.sendRequest(context,spaceCustomBiz2.sendSingleMessage(context.getString(R.string.str_show_share_mm), bitmap, url,
							String.format(context.getString(R.string.str_show_share_content), otherUid)));
				}
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 0:
				space.shareToQQ(context, mTencent, TextUtils.isEmpty(qString) ? BAConstants.SHARE_IMG : qString, context.getString(R.string.str_show_share_mm), url,
						String.format(context.getString(R.string.str_show_share_content), otherUid), mHandler);
				break;
			case 1:
				if (isWXAppInstalledAndSupported(mWXapi)) {
					space.shareToWX(context, String.format(context.getString(R.string.str_show_share_content), otherUid), url,
							BaseBitmap.bmpToByteArray(bitmap, true), SendMessageToWX.Req.WXSceneSession,
							context.getString(R.string.str_show_share_mm), mWXapi);
				} else {
			...
			case 2:
				if (isWXAppInstalledAndSupported(mWXapi)) {
					space.shareToWX(context, String.format(context.getString(R.string.str_show_share_content), otherUid), url,
							BaseBitmap.bmpToByteArray(bitmap, true), SendMessageToWX.Req.WXSceneTimeline,
							context.getString(R.string.str_show_share_mm), mWXapi);
				} else {
					BaseUtils.showTost(context, R.string.str_wetchat_install);
				}
			...
			case 4:
				...
				break;
			case 5:
				space.shareToQZone(context, mTencent, TextUtils.isEmpty(qString) ? BAConstants.SHARE_IMG : qString, context.getString(R.string.str_show_share_mm), url,
						String.format(context.getString(R.string.str_show_share_content), otherUid), mHandler);
				break;
			case 20:
				if (msg.arg1 == 0) {
					...
					if (msg.arg2 == 0 || msg.arg2 == 5) {
						handler.sendMessage(handler.obtainMessage(msg.arg2, 0, -1, shareUrl));
					} else {
						...
						if (file != null && file.exists()) {
							...
							req.getLoadingPicShow(context, (String) msg.obj, 0, msg.arg2, ShareShowDialog.this);
						}
					}
				} else {
					handler.sendMessage(handler.obtainMessage(msg.arg2, 0, -1, shareUrl));
