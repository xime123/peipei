...
/**
 * @Title: SolitaireGrapRedPacketSuccessDialog.java 
 *
 * @Description: 抢红包成功信息弹框
 *
 * @author DYH  
 *
 ...
 * @version V1.0   
 */
public class SolitaireGrapRedPacketSuccessDialog extends Dialog implements DialogInterface.OnDismissListener, android.view.View.OnClickListener {
	...
	public SolitaireGrapRedPacketSuccessDialog(Activity context) {
		super(context);
		this.context = context;
	}
	
	public SolitaireGrapRedPacketSuccessDialog(Activity context, int theme, RedPacketBetCreateInfo checkRedpacketInfo) {
		super(context, theme);
		this.context = context;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.dialog_grap_solitaire_packet_success);
		...
		if(checkRedpacketInfo != null){
			if(checkRedpacketInfo.userInfo != null){
				if(checkRedpacketInfo.orggoldcoin.intValue() > 0){
					tv_money_type.setText(context.getString(R.string.str_open_solitaire_redpacket_success, context.getString(R.string.gold_money)));
				}else{
					tv_money_type.setText(context.getString(R.string.str_open_solitaire_redpacket_success, context.getString(R.string.silver_money)));
				}
