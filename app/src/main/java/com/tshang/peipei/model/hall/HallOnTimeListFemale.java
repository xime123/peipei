 ...
 * @version V1.3.0   
 */
public class HallOnTimeListFemale extends HallBase {
	private boolean isRefresh = true;//区分是刷新状态还是夹在更多状态
	private int startLoad = 0;//加载位置

	public HallOnTimeListFemale(Activity activity, BAHandler mHandler) {
		super(activity);
	...
	public void getHallListData(boolean isUseCacheData, boolean isRefresh) {//女性日榜
		...
		if (isUseCacheData) {
			ThreadPoolService.getInstance().execute(new Runnable() {
				...
				@Override
				public void run() {
					String suffix = HallRankCacheViewData.HALL_ONTIME_FEMALE_FILE_NAME_START;
					...
					if (cacheFile != null) {
						...
						mHandler.sendMessage(mHandler.obtainMessage(HandlerValue.HALL_GET_ONTIME_LIST_FEMALE_SUCCESS_VALUE, -1, REFRESH_CODE, lists));//通过handler把数据传到UI界面去更新
						if (!ConfigCacheUtil.getUrlCacheisEffective(cacheFile, ConfigCacheModel.CONFIG_CACHE_MODEL_SHORT)) {
							getHallUserList(Gender.FEMALE.getValue(), startLoad);//加载女性最新在线
						}
					} else {
						getHallUserList(Gender.FEMALE.getValue(), startLoad);//加载女性最新在线
					}
	...
	public void getOnlineUserListCallBack(int retCode, int isEnd, UserAndSkillInfoList list) {
		if (retCode == 0) {//网络请求数据成功回调
			...
			if (isRefresh) {
				...
				mHandler.sendMessage(mHandler.obtainMessage(HandlerValue.HALL_GET_ONTIME_LIST_FEMALE_SUCCESS_VALUE, isEnd, REFRESH_CODE, list));//通过handler把数据传到UI界面去更新
				HallRankCacheViewData.setGoGirlUserInfoAndSkillListCacheData(activity, HallRankCacheViewData.HALL_ONTIME_FEMALE_FILE_NAME_START, list);
			} else {
				mHandler.sendMessage(mHandler.obtainMessage(HandlerValue.HALL_GET_ONTIME_LIST_FEMALE_SUCCESS_VALUE, isEnd, LOADMORE_CODE, list));
			}
		} else {
			mHandler.sendEmptyMessage(HandlerValue.HALL_GET_ONTIME_LIST_FEMALE_FAILED_VALUE);//加载失败处理
		}
